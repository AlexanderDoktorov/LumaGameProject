...
void UHelpfulFunctionsBPLibrary::ApplyPendulumForce(ACharacter* Character, const FVector& AnchorPoint, float DesiredDistance, float HorizontalStrengthMultiplier, float VerticalStrengthMultiplier, float DampingRatio, float DeltaTime)
{
	...
	// Obliczanie siіy w poziomie
	FVector HorizontalForce = ToAnchor;
	HorizontalForce.Z = 0; // Usuwamy skіadow№ pionow№
	...
	// Obliczanie siіy w pionie
	float VerticalDistanceError = ToAnchor.Z; // Poї№dana odlegіoњж pionowa to 0, wiкc bі№d to po prostu ToAnchor.Z
	FVector CorrectiveVerticalForce = FVector::UpVector * VerticalStrengthMultiplier * VerticalDistanceError;

	// Obliczanie tіumienia
	FVector Velocity = Character->GetVelocity();
	FVector DampingForce = -Velocity * DampingRatio * Character->GetCharacterMovement()->Mass;

	// Oddzielamy siіy tіumienia
	FVector DampingForceHorizontal = DampingForce;
	DampingForceHorizontal.Z = 0; // Tіumienie pionowe bкdzie oddzielnie

	// Ј№czymy siіy w poziomie
	FVector FinalHorizontalForce = CorrectiveHorizontalForce + DampingForceHorizontal;

	// Siіa koсcowa - siіy w poziomie plus koryguj№ca siіa w pionie (uwzglкdniaj№c grawitacjк)
...
float UHelpfulFunctionsBPLibrary::CustomInterpTo(float Current, float Target, float DeltaTime, float StartSpeed, float EndSpeed)
{
	...
	// Obliczanie procentowej odlegіoњci od celu
	const float Alpha = FMath::Clamp(Distance / (Target - StartSpeed), 0.0f, 1.0f);

	// Obliczanie aktualnej prкdkoњci interpolacji na podstawie procentowej odlegіoњci do celu
	const float InterpSpeed = FMath::Lerp(StartSpeed, EndSpeed, Alpha * Alpha); // Kwadrat Alpha, aby dodaж efekt przyspieszenia

	// Obliczanie nowej wartoњci
...
{
	// Zabezpieczenie przed dzieleniem przez zero i nieprawidіowymi wartoњciami
	if (DeltaTime == 0.f || InterpSpeed <= 0.f)
	{
		return Current;
	}

	// Obliczanie siіy interpolacji dla kaїdej skіadowej niezaleїnie
	auto InterpSingle = [DeltaTime, InterpSpeed](float CurrentSingle, float TargetSingle) -> float {
		...
		// Sprawdzanie, czy jesteњmy wystarczaj№co blisko celu
		if (FMath::IsNearlyZero(Distance))
		{
			return TargetSingle;
		}

		// 'Alpha' kontroluje ksztaіt krzywej interpolacji
	...
	// Interpolacja dla kaїdej osi niezaleїnie
	return FVector(
