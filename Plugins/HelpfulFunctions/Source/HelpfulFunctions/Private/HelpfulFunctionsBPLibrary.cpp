...
bool UHelpfulFunctionsBPLibrary::GetIsEnemyState(AActor* Checker, const AActor* ActorToCheck)
{
	if (!IsValid(ActorToCheck) || !IsValid(Checker))
		return false;
	
	FString ClassName = UKismetSystemLibrary::GetClassDisplayName(ActorToCheck->GetClass());
...
void UHelpfulFunctionsBPLibrary::ApplyPendulumForce(ACharacter* Character, const FVector& AnchorPoint, float DesiredDistance, float HorizontalStrengthMultiplier, float VerticalStrengthMultiplier, float DampingRatio, float DeltaTime)
{
	...
	float VerticalDistanceError = ToAnchor.Z; // Po��dana odleg�o�� pionowa to 0, wi�c b��d to po prostu ToAnchor.Z
	...
	DampingForceHorizontal.Z = 0; // T�umienie pionowe b�dzie oddzielnie

	// ��czymy si�y w poziomie
	FVector FinalHorizontalForce = CorrectiveHorizontalForce + DampingForceHorizontal;

	// Si�a ko�cowa - si�y w poziomie plus koryguj�ca si�a w pionie (uwzgl�dniaj�c grawitacj�)
